...
int main(int argc, char **argv) {
  ...
  nh.param("planning/vel", _Vel, 1.0);//3
  nh.param("planning/acc", _Acc, 1.0);//2
  nh.param("planning/dev_order", _dev_order, 3);//4
  nh.param("planning/min_order", _min_order, 3);//3
  nh.param("vis/vis_traj_width", _vis_traj_width, 0.15);//0.07
  nh.param("map/resolution", _resolution, 0.2);//0.2
  nh.param("map/x_size", _x_size, 50.0);//30
  nh.param("map/y_size", _y_size, 50.0);//30
  nh.param("map/z_size", _z_size, 5.0);//4
  nh.param("path/resolution", _path_resolution, 0.05);//0.2
  nh.param("replanning/thresh_replan", replan_thresh, -1.0);//1.5
  nh.param("replanning/thresh_no_replan", no_replan_thresh, -1.0);//2.0

  _poly_num1D = 2 * _dev_order;//8
  ...
  // set the obstacle map
  _map_lower << -_x_size / 2.0, -_y_size / 2.0, 0.0;//-15,-15,0
  _map_upper << +_x_size / 2.0, +_y_size / 2.0, _z_size;//15,15,4
  _inv_resolution = 1.0 / _resolution;//1/0.2=5
  _max_x_id = (int)(_x_size * _inv_resolution);//30*5=150
  _max_y_id = (int)(_y_size * _inv_resolution);//30*5=150
  _max_z_id = (int)(_z_size * _inv_resolution);//4*15=60
