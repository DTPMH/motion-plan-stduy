...
void camerclouds_2_worldclouds()
{
    while(ros::ok())
    {
        ...
        /***********************************由定位结果计算转换矩阵***************************************************************/ 
                ...
                R=R*R0;//将相机坐标系与世界坐标系对准是的旋转加入到转换矩阵中,小觅相机需要使用的语句
                ...
                pass_filter.setFilterLimits(0.5,5.0);
...
*/                
                ...
                voxel_filter.setInputCloud(Camera_cloud_pass_filter);
                voxel_filter.filter(*Camera_cloud);
                cout<<"体素滤波后的点云数量："<<Camera_cloud->points.size()<<endl;
                ros::Time time5=ros::Time::now();
                cout<<"点云滤波花费的时间"<<(time5-time4).toSec()<<endl;
                
                /*
                该段语句可被Eigen::AngleAxisd R0(M_PI,Eigen::Vector3d (1,0,0));代替
        
                //将滤波后的相机坐标下的点云转换到VINS的坐标系下，即使两者的坐标系相同。
                pcl::PointXYZRGB pt;
                Camera_VINS_cloud.clear();
                for (int i = 0; i < Camera_cloud.points.size(); ++i)
                {
                //第一列注释的表示相机正方向是Y轴，且不考虑转换至世界坐标系下，第二列注释的是相机正方向是X轴,且不考虑转换至世界坐标系下
                //第三列注释表示不考虑VINS初始定位姿态的影响，将点云完全转换到世界坐标系（认为相机朝向X轴）下， 
                //第四列注释表示不考虑VINS初始定位姿态的影响，将点云完全转换到世界坐标系（认为相机朝向Y轴）下，  
                    pt.x=Camera_cloud[i].x;//x*****z*******z****x
                    pt.y=-Camera_cloud[i].y;//z****-x*****-y***-y
                    pt.z=-Camera_cloud[i].z;//-y***-y****x****-z
                    pt.r=Camera_cloud[i].r;
                    pt.g=Camera_cloud[i].g;
                    pt.b=Camera_cloud[i].b;
                    Camera_VINS_cloud.push_back(pt);

                }
                 cout<<"转换坐标系后的点云数量："<<Camera_VINS_cloud.points.size()<<endl;
                */
...
int main(int argc, char **argv)
{
    ...
    //ros::Subscriber points_camera_sub = n.subscribe("/mynteye/points/data_raw",1,points_camera_Callback);//使用小觅相机是用到的语句
    ros::Subscriber points_camera_sub = n.subscribe("/camera/depth/color/points",1,points_camera_Callback);//使用D435i时用到的语句
    pub_cloud = n.advertise<sensor_msgs::PointCloud2>("/pcl_render_node/cloud", 10);
